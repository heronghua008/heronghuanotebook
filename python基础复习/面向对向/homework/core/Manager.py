...
class Manager:
    ...
    def createClasses(self):
        ...
        # 创建一个属于这个班级的文件用存储学生信息,将文件的路径存储到班级对象中
        # 创建一个班级对象(名称 学校 学科对象 讲师空列表 学生信息所在文件的路径),dump进class
        pass
    ...
    def boundClass(self):
        # 管理员选择为老师还是学生指定班级
        # 如果是为老师绑定班级:
            # 找到指定老师和对应的班级(都是通过show方法查看之后选择)
            # 给讲师对象的班级属性的列表中加入一个新的项,值为班级的对象
            # 给班级对象中的讲师属性列表加入一个新和项,值为讲师的对象
        # 如果为学生绑定班级
            # 找到指定学生和对应班级(都是通过show方法查看之后选择)
...
# 首先 以管理员的身份登录
# 登录之后 就应该实例化一个对应的对象 manage_obj = Manager(name)
# 管理员对象可以调用所有的方法
